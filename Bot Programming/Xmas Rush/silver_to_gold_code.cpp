/*
  Inspired by - https://github.com/WildSmilodon/codingame-puzzle-solutions/blob/master/bots/xmas-rush/xmas-rush.cpp
*/
...
public:
  ...
  void push (int rc, int dir, int pId) { // push one row or column in dir
    if (dir == UP) { // push rc column up
      ...
      setGridTile(grid[N-1][rc], player[pId] & 127); // copy only 7 bits
    ...
    if (dir == RIGHT) { // push rc row right
      int cell = grid[rc][N-1]; // cell to wrap around
      for (int i = N - 1; i > 0; i--) {
    ...
    if (dir == DOWN) {
      int cell = grid[N-1][rc]; // cell to wrap around
      for (int i = N-1; i > 0; i--) {
        grid[i][rc] = grid[i-1][rc]; // move each cell
      }
      // copy hand tile to grid
    ...
    if (dir == LEFT) {
      ...
      // copy hand tile to grid
      setGridTile(grid[rc][N-1], player[pId] & 127); // copy only 7 bits
      // copy grid tile to hand
      setPlayerTile(player[pId], cell & 127); // copy only 7 bits
      // teleport player
      for (int k = 0; k < 2; k++) {
...
void pushOnce (Game game, int pId, int &rc, int &dir, int &maxScore) {
  maxScore = -1000000;
  for (int i = 0; i < N; i++) {
    for (int d = 0; d < 4; d++) {
      Game g = game;
...
void pushOnceOnce (Game game, int pId, int &rc, int &dir, int &maxScore) {
  maxScore = -1000000;
  for (int i = 0; i < N; i++) {
    for (int d = 0; d < 4; d++) {
      Game g = game;
      g.push(i, d, pId);
      int s = g.score(pId);
      int rc2, dir2, maxScore2;
      pushOnce(g, pId, rc2, dir2, maxScore2);
      s = 10*s + maxScore2;
      // cerr << rc << " " << d << " s= " << s << " " << maxScore2 << endl;
      if (s > maxScore) {
        maxScore = s;
        rc = i;
        dir = d;
...
void pushTwiceOnce (Game game, int pId, int &rc, int &dir, int &maxScore) {
  int Trc = 0, Tdir = 0, Ts = -1;
  Game g = game;

  pushOnce(g, otherPId(pId), Trc, Tdir, Ts); // find what he will do
  g.push(Trc, Tdir, otherPId(pId)); // make his move

  pushOnceOnce(g, pId, Trc, Tdir, Ts); // consider my move
...
void whereToMove (Game game, int pId, int &Trow, int &Tcol) {
  int rc = 0, dir = 0, maxScore = -1000000;
  // game.BFS(Trow, Tcol);
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      if (getGridDist(game.grid[i][j]) < getPlayerStepsLeft(game.player[pId])) {
        ...
        int score;
        pushOnce(g, pId, rc, dir, score);
        if (score > maxScore) {
          maxScore = score;
          Trow = i;
          Tcol = j;
        }
        // cerr << i << " " << j << " " << score << " " << maxScore << " " << Trow << " " << Tcol << " trc " << rc << " " << dir << endl;
      }
    }
  }
}
...
int main() {
  ...
  while (1) {
    ...
    if (game.turnType == PUSH_TURN) {
      ...
      if (gridLockedCounter < 5) { // deadlock prevention (more solid condition required) (like gridLockedCounter < 5 || my player num cards < other player numcards)
