...
struct Player {
    ...
    int cards_count;
    int row, col;
    string tile; // tile in hand
    vector<pair<int, int>> goals;
    Item item; // item on the tile in hand
...
void do_push(Push push, vector<vector<string>> &grid, Player &p, vector<Item> &items) {
    if (push.is_row) {
        ...
        for (auto &item : items) {
            if (item.row < 0) { // in hand item
                item.row = row;
                item.col = push.is_right_down ? 0 : N - 1;
            } else if (item.row == row) { // item on the same row
                item.col += push.is_right_down ? 1 : -1;
                if (item.col == N || item.col < 0) {
                    // item.row = item.col = p.item.row; // -1 or -2 according to player
                    // p.item = item;
                }
            }
        }
        for (auto &goal : p.goals) {
            if (goal.first < 0) { // in hand item
                goal.first = row;
                goal.second = push.is_right_down ? 0 : N - 1;
            } else if (goal.first == row) { // goal on the same row
                goal.second += push.is_right_down ? 1 : -1;
                if (goal.second == N || goal.second < 0) {
                    // goal.first = goal.second = p.item.row; // -1 or -2 according to player
                }
            }
        }
    } else {
        ...
        for (auto &item : items) {
            if (item.row < 0) { // in hand item
                item.row = push.is_right_down ? 0 : N - 1;
                item.col = col;
            } else if (item.col == col) { // item on the same col
                item.row += push.is_right_down ? 1 : -1;
                if (item.row == N || item.row < 0) {
                    // item.row = item.col = p.item.row; // -1 or -2 according to player
                    // p.item = item;
                }
            }
        ...
        for (auto &goal : p.goals) {
            if (goal.first < 0) { // in hand item
                goal.first = push.is_right_down ? 0 : N - 1;
                goal.second = col;
            } else if (goal.second == col) { // goal on the same col
                goal.first += push.is_right_down ? 1 : -1;
                if (goal.first == N || goal.first < 0) {
                    // goal.first = goal.second = p.item.row; // -1 or -2 according to player
                }
            }
        }
    ...
    for (int i = 0; i < N; i++) {
        for (int is_right_down = 0; is_right_down <= 1; is_right_down++) {
            for (int is_row = 0; is_row <= 1; is_row++) {
                do_push(Push(i, is_right_down, is_row), grid, me, items);
