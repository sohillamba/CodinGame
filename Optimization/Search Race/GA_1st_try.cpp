...
// for Car simulations doubles are used for precision, to match simulation with the CG's simulation doubles are used

int gameType = 0;
const int PC = 0;

const double H = 9000;
const double W = 16000;
const double PI = 3.14159265358979323846;
const double MAX_OUTSIDE_RANGE = 0;
const double MIN_DIS_TO_CLEAR_CP = 300;
...
public:
    ...
    */
    void simulate1Step(const double &angleChange, const double &powerChange) {
        prevAngle = angle;
        ...
        if (getDistance(pos, checkpoint[nextCpIdx]) < MIN_DIS_TO_CLEAR_CP) {
            nextCpIdx = (nextCpIdx + 1) % checkpointsCount;
            reachedCpCount++;
        }
    }
    ...
    double getScore() const {
        double score = 0;
        score += 1000.0 * reachedCpCount;

        double d = getDistance(pos, checkpoint[nextCpIdx]);
        double disPen = d / 50000.0; // 2 times diagonal dis

        score += max(0.0, 100.0 - disPen);
...
const int ANGLE_CHANGE = 18;
const int POWER_CHANGE = 200;
...
public:
    ...
    void ramdomizeIthGene(const int &i) {
        angle[i] = randomInt(-ANGLE_CHANGE, ANGLE_CHANGE);
        power[i] = randomInt(-POWER_CHANGE, POWER_CHANGE);
    }
    ...
    float calculateFitness() const {
        Car car(initialCar);
        for (int i = 0; i < GENE_SIZE; i++) {
            car.simulate1Step(angle[i], power[i]);
...
public:
    ...
    //     float randomValue = randomDouble(0.0, cumulativeSum[POPULATION_SIZE - 1]);
...
void solveOnSite() {
    ...
    for (int turn = 0; ; turn++) {
        ...
        // double angleChange = randomInt(-18, 18);
        // double powerChange = randomInt(50, 120);
        double angleChange = ansPath.angle[turn];
        double powerChange = ansPath.power[turn];
        initialCar.simulate1Step(angleChange, powerChange);
        // initialCar.changeAngleAndPowerBy(angleChange, powerChange);

        cout << "EXPERT " << angleChange << " " << initialCar.power << endl;
    }
}
