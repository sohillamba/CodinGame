/*
    Implemented fast state operations and minimax algorithm with alpha beta pruning
      Goes up to depth 6 (3~4 without GCC optimization), and 2~5k simulations on average (peak 25k simulations in some cases)
...
public:
    ...
    vector<Action> getAllActions(int pId) {
        ...
        */
        ...
        // TODO: check: should not come in this function if player is waiting

        // COMPLETE actions
        if (state.getPlayersSun(pId) >= completionCost()) {
            uint64_t trees = getPlayersActiveTrees3(state, pId);
            for (int i = 0; i < N; i++) {
                if ((trees >> i) & 1ULL) {
        ...
        if (state.getPlayersSun(pId) >= growCost(2, cntPlayersTrees3(state, pId))) {
            uint64_t trees = getPlayersActiveTrees2(state, pId);
            for (int i = 0; i < N; i++) {
                if (((trees >> i) & 1ULL)) {
        ...
        if (state.getPlayersSun(pId) >= growCost(1, cntPlayersTrees2(state, pId))) {
            uint64_t trees = getPlayersActiveTrees1(state, pId);
            for (int i = 0; i < N; i++) {
                if (((trees >> i) & 1ULL)) {
                    actions.push_back(Action(GROW, i));
                }
        ...
        if (state.getPlayersSun(pId) >= growCost(0, cntPlayersTrees1(state, pId))) {
            uint64_t seeds = getPlayersActiveSeeds(state, pId);
            for (int i = 0; i < N; i++) {
                if (((seeds >> i) & 1ULL)) {
                    actions.push_back(Action(GROW, i));
                }
        ...
        // SEEDING actions
        if (cntPlayersSeeds(state, pId) < 3 && state.getPlayersSun(pId) >= seedingCost(cntPlayersSeeds(state, pId))) {
            ...
            for (int i = 0; i < N; i++) {
                    ...
                    if ((trees1 >> i) & 1ULL) { // seeding from size 1 tree
                        for (int j : REACHABLE_CELLS[i][1]) {
                            if ((seedingLocations >> j) & 1ULL) {
                                actions.push_back(Action(SEED, i, j));
                            }
                        }
                    }
                }
                else if ((trees2 >> i) & 1ULL) { // seeding from size 2 tree
                    for (int j : REACHABLE_CELLS[i][2]) {
                        if ((seedingLocations >> j) & 1ULL) {
                            actions.push_back(Action(SEED, i, j));
                        }
                ...
                else if ((trees3 >> i) & 1ULL) { // seeding from size 3 tree
                    for (int j : REACHABLE_CELLS[i][3]) {
                        if ((seedingLocations >> j) & 1ULL) {
                            actions.push_back(Action(SEED, i, j));
                        }
